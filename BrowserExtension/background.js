...
// MV3 uses a service worker, where window/document do not exist. Maxwell
// deliberately ships a Chromium-family browser, so report Chrome directly.
var isOpera = false;
var isFirefox = false;
var isIE = false;
var isEdge = false;
...
async function OnPortMessage(message) {
    try {
        ...
        if (message.functionName === "openrpautilscript") {
            // content.js is declared statically in the MV3 manifest and is
            // therefore already present in each permitted page/frame.
            // Do not execute Host-provided script strings: MV3 prohibits it.
            openrpautil_script = "bundled-content-script";
            return;
        }
        ...
        if (message.functionName === "executescript") {
            message.error = "MAXWELL_MV3_DYNAMIC_SCRIPT_UNSUPPORTED: Chrome Manifest V3 blocks arbitrary JavaScript strings supplied by a workflow. Use OpenRPA browser activities (GetElement, ClickElement, SendKeys, GetTableData) or add reviewed static code to the Maxwell extension.";
...
async function tabsOnUpdated(tabId, changeInfo, tab) {
    ...
    try {
        ...
        // content.js is injected statically by the MV3 manifest.
    } catch (e) {
    //    console.log(e);
    //    console.log(tab);
        return;
...
// Retained for source compatibility with OpenRPA addon2. It accepts only
// packaged files or functions; arbitrary `code` strings are rejected by MV3.
var tabsexecuteScript = async function (tabid, options) {
    if (options && options.code) {
        throw new Error("MAXWELL_MV3_DYNAMIC_SCRIPT_UNSUPPORTED");
    }
    var target = { tabId: tabid };
    if (options && options.frameId !== undefined) target.frameIds = [options.frameId];
    else if (options && options.allFrames) target.allFrames = true;
    var request = { target: target };
    if (options && options.files) request.files = options.files;
    if (options && options.func) request.func = options.func;
    if (options && options.args) request.args = options.args;
    return await chrome.scripting.executeScript(request);
};
...
function AttachRuntimePortListeners() {
    if (port == null || port === undefined || typeof OnPortMessage !== "function") return;
    try {
        port.onMessage.removeListener(OnPortMessage);
        port.onDisconnect.removeListener(OnPortDisconnect);
    } catch (e) {
        // The first connection has no prior listeners to remove.
    }
